...
function FLANNNNDS(nodes::ExplicitSampleSet{S}, bvp::GeometricSteering; max_neighbor_count=1000) where {S}
    ...
    inds = Vector{Cint}(undef, max_neighbor_count)
    dists = Vector{T}(undef, max_neighbor_count)
    nnds = FLANNNNDS(FLANN.flann(Array(vecs2mat(nodes.V)), params, FLANN.Euclidean()), inds, dists)
...
addstates!(nnds::FLANNNNDS, x::State) = FLANN.addpoints!(nnds.index, x)
addstates!(nnds::FLANNNNDS, xs::AbstractVector{<:State}) = FLANN.addpoints!(nnds.index, vecs2mat(xs))

function neighbors(nnds::FLANNNNDS, nodes::ExplicitSampleSet, bvp::GeometricSteering, v::Int, r;
...
function neighbors(nnds::FLANNNNDS, nodes::ExplicitSampleSet, bvp::GeometricSteering, x::State, r;
    ...
    # gen = dir === Val(:F) ? ((index=Int(i), bvp(x, nodes[Int(i)], r)...) for i in near_indices if !omit(i)) :
    #                         ((index=Int(i), bvp(nodes[Int(i)], x, r)...) for i in near_indices if !omit(i))
