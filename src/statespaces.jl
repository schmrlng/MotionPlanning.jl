...
export sample_space, state2workspace, waypoints, workspace_waypoints, collision_waypoints
...
for wpf in (:waypoints, :workspace_waypoints, :collision_waypoints)
    @eval $wpf{S<:State}(v::S, w::S, SS::StateSpace) = $wpf(v, w, SS.dist, SS.s2w)
end
...
function plot_tree(V::Path, A::Vector{Int}, SS::StateSpace; kwargs...)    # V is a vector of states, i.e. a Path
    VW = [state2workspace(v, SS.s2w) for v in V]
    pts = hcat(VW[find(A)]...)
