...
export NearNeighborCache, MetricNN, Neighborhood, filter_neighborhood, inball, knn, inball!, knn!, mutualknn!, addpoints
...
### Metric brute force
type MetricNN_BruteForce{S<:State,T<:FloatingPoint} <: MetricNN
    V::Vector{S}
    DS::Matrix{T}
    ...
    MetricNN_BruteForce(V::Vector{S}, dist::Metric = Euclidean()) =
        new(V, pairwise(dist, hcat(V...)), Array(Neighborhood{T}, length(V)), zeros(T, length(V)), dist)
end
MetricNN_BruteForce{S<:State}(V::Vector{S}, dist::Metric = Euclidean()) = MetricNN_BruteForce{S,eltype(S)}(V,dist) # so constructor works without {}
...
### Euclidean k-d tree
type EuclideanNN_KDTree{S<:AbstractVector,T<:FloatingPoint} <: MetricNN
    V::Vector{S}
    DS::KDTree
    cache::Vector{Neighborhood{T}}
    kNNr::Vector{T}
    ...
    function EuclideanNN_KDTree(V::Vector{S}, dist::Metric = Euclidean())
        ...
        new(V, KDTree(hcat(V...)), Array(Neighborhood{T}, length(V)), zeros(T, length(V)), Euclidean()) # TODO: leafsize, reorder?
    end
end
EuclideanNN_KDTree{S<:State}(V::Vector{S}, dist::Metric = Euclidean()) = EuclideanNN_KDTree{S,eltype(S)}(V,dist) # so constructor works without {}
